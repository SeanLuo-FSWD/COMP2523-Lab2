...
class ReminderBook {
  ...
  listNotes() {
    if (this.noteList.length == 0) {
      ...
      Object.entries(this._listObj()).forEach(([tag, itemObj]) => {
        console.log(`\n[Tag]: ${tag} \n`);
  ...
  searchNote(term: string) {
    ...
    Object.entries(listObj).forEach(([tag, itemObj]) => {
    ...
    if (!tagFound) {
      // if no tag found above, proceed to search body value
      Object.values(listObj).forEach((itemObj) => {
        // looping tag level

        itemObj.forEach((item) => {
          // looping item-obj array
          if (
            item.body.toUpperCase() === term.toUpperCase() ||
            item.body.toUpperCase().includes(term.toUpperCase())
          ) {
            if (!item.done) {
              console.log(`⭕️ ${item.body}\n`);
            } else {
